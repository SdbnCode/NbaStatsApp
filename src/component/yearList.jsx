...
function YearList({ setYear }) {
  ...
  const handleYearChange = (year) => {
    ...
    setYear(year !== "Select a Year" ? year : null);
  };

  return (
    <Listbox value={selectedYear} onChange={handleYearChange}>
      <div className="relative w-auto mt-2">
        <ListboxButton className="block w-full cursor-default rounded-md bg-white py-1.5 pl-1.5 pr-10 text-left text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 focus:outline-none focus:ring-2 focus:ring-black sm:text-sm sm:leading-6">
